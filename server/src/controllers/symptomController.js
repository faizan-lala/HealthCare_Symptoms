...
const createSymptom = async (req, res) => {
  try {
    ...
    try {
      ...
      if (io) {
        io.to(`user:${req.user.id}`).emit('symptom:created', { symptom });
      }
    } catch (_) {}
...
const updateSymptom = async (req, res) => {
  try {
    ...
    try {
      const io = req.app.get('io');
      if (io) {
        io.to(`user:${req.user.id}`).emit('symptom:updated', { symptom });
      }
    } catch (_) {}
  } catch (error) {
...
const deleteSymptom = async (req, res) => {
  try {
    ...
    try {
      const io = req.app.get('io');
      if (io) {
        io.to(`user:${req.user.id}`).emit('symptom:deleted', { id: req.params.id });
      }
    } catch (_) {}
  } catch (error) {
